...
ower of Hanoi. You should assume that the rods are numbered, with the 
first rod being 1, the second (auxiliary) rod being 2, and the last 
(goal) rod being 3.
...
def Hanoi(n,from_nod,to_nod,mid_nod):
    nonlocal lst
    if n==1:
        lst.append('1')
        return
    else:
        Hanoi(n-1,from_nod,mid_nod,to_nod)
        lst.append('1')
        Hanoi(n-1,mid_nod,to_nod,from_nod)
        return
def Game(n):
    lst=[]
    Hanoi(n,'A','B','C')
